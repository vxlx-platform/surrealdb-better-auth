...
const classifyQueryError = (err: unknown): QueryErrorClassification => {
  ...
  if (
    UNIQUE_CONSTRAINT_RE.test(msg) ||
    (err instanceof ServerError && err.kind === "AlreadyExists")
  )
    return { kind: "unique" };
  return { kind: "none" };
};
...
export const createQueryErrorWrapper = (adapterError: AdapterErrorFactory) => {
  const MAP = new Map<any, string>([
    ...
    [
      InvalidSessionError,
      "session is invalid. The active transaction/session may have been closed",
    ],
  ]);
  ...
  return (err: unknown, ctx: string): never => {
    ...
    if (classification.kind === "field")
      throw adapterError(`Invalid value for field "${classification.field}" while ${ctx}.`, err);
    if (classification.kind === "unique")
      throw adapterError(`Unique constraint violation while ${ctx}.`, err);
    if (err instanceof SurrealError)
      throw adapterError(`SurrealDB error while ${ctx}: ${err.message}`, err);
    throw adapterError(
      `SurrealDB query failed while ${ctx}: ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  };
};
